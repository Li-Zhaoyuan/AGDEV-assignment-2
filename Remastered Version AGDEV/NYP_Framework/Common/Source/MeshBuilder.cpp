...
Mesh* MeshBuilder::GenerateQuadXZ(const std::string &meshName, Color color, float length)
{
    Vertex v;
    std::vector<Vertex> vertex_buffer_data;
    std::vector<GLuint> index_buffer_data;
...
{
    for (std::map<std::string, Mesh*>::iterator it = meshMap.begin(), end = meshMap.end(); it != end; ++it)
    {
        delete it->second;
    }
    meshMap.clear();
